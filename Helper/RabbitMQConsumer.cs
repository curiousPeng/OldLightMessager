...
namespace LightMessager.Helper
{
    ...
    */
    public sealed class RabbitMQConsumer : IRabbitMQConsumer
    {
        ...
        private static void DirectEnsureQueue(IModel channel, Type messageType, out string exchange, out string routeKey, out string queue)
        {
        ...
        private static void DirectEnsureQueue(IModel channel, ref string exchange, ref string queue, string routeKey)
        {
            string key = routeKey;//自定义exchange，queue，routekey，就默认用routekey是唯一标准

            if (!dict_info_custom.ContainsKey(key))
            {
                var info = GetQueueInfoForCustom(key,exchange,queue,routeKey);
                exchange = info.Exchange;
                routeKey = info.RouteKey;
                queue = info.Queue;
