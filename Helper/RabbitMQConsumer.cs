...
namespace LightMessager.Helper
{
    ...
    */
    public sealed class RabbitMQConsumer : IRabbitMQConsumer
    {
        ...
        public void RegisterDirectHandler<TMessage, THandler>(string exchangeName = "", string queueName = "", string routeKey = "", bool IsNeedAck = true, bool redeliveryCheck = false)
        ...
        public void RegisterTopicHandler<TMessage, THandler>(string routeKey, string exchangeName = "", string queueName = "", bool IsNeedAck = true, bool redeliveryCheck = false, params string[] subscribePatterns)
            where THandler : BaseHandleMessages<TMessage>
            where TMessage : BaseMessage
        {
            ...
            try
            {
                var key = (typeof(TMessage), routeKey);
                if (!dict_func_name.ContainsKey(key))
                {
                    var handler = dict_func_name.GetOrAdd((typeof(TMessage), routeKey), p => Activator.CreateInstance<THandler>()) as THandler;
                    var channel = connection.CreateModel();
                    var consumer = new EventingBasicConsumer(channel);
                    /*
                      @param prefetchSize maximum amount of content (measured in octets) that the server will deliver, 0 if unlimited
                    ...
                    */
                    ...
                    var exchange = string.Empty;
                    var queue = string.Empty;
                    EnsureQueue(channel, typeof(TMessage), routeKey, out exchange, out queue, subscribePatterns);
                    channel.BasicConsume(queue, false, consumer);
                }
            }
            catch (Exception ex)
        ...
        /// <typeparam name="TMessage">消息类型</typeparam>
        /// <typeparam name="THandler">消息处理器类型</typeparam>
        public void RegisterFanoutHandler<TMessage, THandler>(string exchangeName = "", string queueName = "", bool IsNeedAck = true, bool redeliveryCheck = false)
            where THandler : BaseHandleMessages<TMessage>
            where TMessage : BaseMessage
        {
            try
